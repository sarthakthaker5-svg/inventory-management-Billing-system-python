...
class productClass:
    def __init__(self, root):
        ...
        title=Label(product_Frame, text="Product Details", font=("Bahnschrift Semi Bold", 15), bg="blue", fg="white",cursor="hand2",relief=RIDGE).pack(side=TOP,fill=X)
        
        lbl_category=Label(product_Frame, text="Category", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=60)
        lbl_supplier=Label(product_Frame, text="Supplier", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=110)
        lbl_product=Label(product_Frame, text="Name", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=160)
        lbl_price=Label(product_Frame, text="Price", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=210)
        lbl_qty=Label(product_Frame, text="Quantity", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=260)
        lbl_status=Label(product_Frame, text="Status", font=("Bahnschrift Semi Bold", 15), bg="white").place(x=30,y=310)
        ...
        # Buttons
        btn_add = Button(product_Frame, text="Save", command=self.add, font=("Bahnschrift Semi Bold", 15), bg="blue",fg="white", cursor="hand2").place(x=10, y=400, width=100, height=40)
        btn_update=Button(product_Frame,text="Update",command=self.update,font=("Bahnschrift Semi Bold",15),bg="red",fg="white",cursor="hand2").place(x=120,y=400,width=100,height=40)
        btn_delete=Button(product_Frame,text="Delete",command=self.delete,font=("Bahnschrift Semi Bold",15),bg="orange",fg="white",cursor="hand2").place(x=230,y=400,width=100,height=40)
        btn_clear=Button(product_Frame,text="Clear",command=self.clear,font=("Bahnschrift Semi Bold",15),bg="yellow",fg="white",cursor="hand2").place(x=340,y=400,width=100,height=40)
    ...
    def add(self):
        ...
        cur = con.cursor()
        try:
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty" or self.var_sup.get() == "Empty" or self.var_name.get() == "":
    ...
    def delete(self):
        ...
        try:
            if self.var_pid.get() == "":
                ...
                return
            if not self.var_pid.get().isdigit():
                messagebox.showerror("Error", "P Id must be numeric", parent=self.root)
                return
            cur.execute("SELECT * FROM product WHERE pid=?", (self.var_pid.get(),))
            row = cur.fetchone()
            if row is None:
